...
def _check_in_lmdb(photo_ids_by_listing, keys):
    for listing_id, photo_ids in photo_ids_by_listing.items():
        for photo_id in photo_ids:
            if get_key(listing_id, photo_id) not in keys:
                raise ValueError(f"{photo_ids_by_listing} is not the LMDB features")
        
def _check_enough_images(photo_ids_by_listing, min_length):
    for listing_id, photo_ids in photo_ids_by_listing.items():
        if len(photo_ids) < min_length:
            raise ValueError(f"Not enough image for listing {listing_id}")
...
def get_caption(photo_id: PhotoId, photo_id_to_caption: Dict[int, Dict]) -> List[int]:
    # Captionless image
    if photo_id not in photo_id_to_caption:
        return []
...
class PrecomputedBnBDataset(Dataset):
    def __init__(
        self,
        precomputed_path: Union[Path, str],
        ...
        self._tokenizer = tokenizer
        self._pairs = load_tokens(precomputed_path, tokenizer, max_instruction_length)
    ...
    def __getitem__(self, index: int):
        ...
        instructions = [positive["instruction_tokens"][0]]
        f, b, p, m = self._get_visual_features(positive["trajectory"])
        features, boxes, probs, masks = [f], [b], [p], [m]

        # get the negative captions
        for pair in negatives:
            instructions += [pair["instruction_tokens"][0]]
            f, b, p, m = self._get_visual_features(pair["trajectory"])
            features += [f]
            boxes += [b]
            probs += [p]
            masks += [m]
